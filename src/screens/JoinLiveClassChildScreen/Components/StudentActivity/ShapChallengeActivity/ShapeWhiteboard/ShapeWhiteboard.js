...
const ShapeChallengeWhiteboard = forwardRef((props, ref) => {
  ...
  //     {
  //       name: "Triangle",
  //       path: "https://check-in-activity.s3.ap-south-1.amazonaws.com/triangle.png",
  //     },
  //     {
  //       name: "Triangle",
  //       path: "https://check-in-activity.s3.ap-south-1.amazonaws.com/triangle.png",
  ...
  function checkAndUpdateWriting(whiteBoardLines) {
    let result = whiteBoardLines[whiteBoardLines.length - 1];

    if (result.tool == "eraser") {
      replaceChildPointWithEraser(result.points);
    }

    if (result.tool == "text") {
      renderChildText(result);
    }

    if (result.tool == "line") {
      renderChildStraightLine(result.points);
    }
    if (result.tool == "pen") {
      replaceChildPoint(result.points, result.colorPickerValue);
    }
    setWhiteboardlines([...whiteBoardLinesfinal, result]);
  }
  ...
  useEffect(() => {
    if (props?.whiteboardPoints?.length) {
      let arr = props?.whiteboardPoints;

      arr = arr.filter((item) => Array.isArray(item) && item.length);

      if (arr.length) {
        // console.log("arr length", arr[arr.length - 1]);
        checkAndUpdateWriting(arr[arr.length - 1]);
      }
    }
  ...
  const handleMouseLeave = () => {
    ...
    setAllLines([...allLines, lines[lines.length - 1]]);

    console.log("allLines length", allLines.length);

    if (allLines.length == 0) {
      props?.sendRemoteWhiteboardActivity(lines);
    } else {
      props?.sendRemoteWhiteboardActivity(allLines);
    }
    ...
    >
      ...
      >
        ...
        <View style={styles.gridStyles}>
          {props?.shapesDraw?.map((item) => {
            return (
              <View style={styles.gridStylesItems}>
                <Image
                  style={{
                    height: 180,
                    width: 130,
...
export default ShapeChallengeWhiteboard;
