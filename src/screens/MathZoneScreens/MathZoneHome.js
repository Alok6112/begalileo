...
import {
  getParam,
  getParamNavigationV5,
} from '../../components/helpers/navigationV5Data';

const MathZoneHome = props => {
  ...
  useEffect(() => {
    const studentId = getParamNavigationV5(props, 'student_id', '');

    console.log('student_id in MathZoneHome', studentId);

    dispatch(getMathZoneQuizzes(studentId));
  }, []);
  ...
  const onPressTagItem = tagItem => {
    const student_id = getParamNavigationV5(props, 'student_id', null);
    props.navigation.navigate(Constants.MATH_ZONE_QUIZ_SCREEN, {
