...
const MyInput = ({handleInputs,val, input_refs,index, isValidated}) =>{
  const [presentValue, setPresentValue] = useState("")
    return<View  style={{height:30,width:50,borderWidth:1,paddingLeft:8,paddingRight:8,borderRadius: 5}}>
        <TextInput
...
const LongMultiplicationKeyingType = ({data, submitResponse, screenType, isResponse, handleStudentAnswerCorrect}) => {
  ...
  console.log("INSIDE KEYINGGGGG");

  return (
      <View>
        <View style={styles.outterSection}>
          <View style={{width: '80%'}}>
            <View style={{marginTop: 10,marginBottom:10,display:"flex",justifyContent:"flex-start",alignItems:"flex-start"}}>
              {/* <HtmlViwer source={{html: question.questionName}} isQuestion={true} isColumnView={true}/> */}
              <QuestionPartBuilder data={question.questionName}/>
            </View>

            <View style={{display:"flex"}}>
                <View style={{display:"flex",width:question.cols*70,alignItems:"center"}}>
                {
                    question.questionContent.map((item,index)=>{
                      ...
                      <View style={[styles.row,{alignItems:"center"}]}>
                        ...
                        item.map((val,i)=>{
                            ...
                            // <OwnTextInput myId={index+""+i} handleInputs={handleInputs} val={val} isValidated={isValidated} height={30} width={50}/>
                              }</View>                                
                                 // return <View style={[styles.row,{display:"flex",justifyContent:"center",alignItems:"center",margin:5}]}><MyInput handleInputs={handleInputs} val={val} input_refs={input_refs} index={item.length*index+i}/></View>                                                                 
                            }
                        })
                        }
                        </View>
...
export default LongMultiplicationKeyingType;