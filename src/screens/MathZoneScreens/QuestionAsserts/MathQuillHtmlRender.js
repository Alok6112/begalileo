...
const MathQuillHtmlRender = ({ content, isQuestion, clickedOption ,isColumnView}) => {
...
content = content.replace("\n", "</br>");
  content = isQuestion
    ? `<div style='font-style: normal;font-size: 18px; font-weight: 700; color:#233584'>${content}</div>`
    : clickedOption
    ? `<div style='font-style: normal;font-size: 18px; font-weight: 700; background: #EDEFFC; padding:10px;min-width:100px; border:2px solid  #D0D4E1; border-radius: 12px;'>${content}</div>`
    : `<div style=' font-style: normal;font-size: 18px; font-weight: 700; padding:10px; border:2px solid  #D0D4E1; border-radius: 12px;'>${content}</div>`;
...
`;
  ...
  return (
    <View style={{ height: webViewHeight, minWidth: 150}}>
      <WebView
        originWhitelist={["*"]}
        source={{ html: outputResponse }}
        //containerStyle={{height:webViewHeight,minWidth:100}}
...
export default MathQuillHtmlRender;
